...
static double json_Infinity()
{
	//static unsigned char t[8]={0x00, 0x00, 0x00, 0x00,   0x00, 0x00, 0xF0, 0x7F};
	static json_uint64_t  t = 0x7FF0000000000000;
	double r = 0;
					  ...
					  const char *str, int len)
{
  ...
  redo_char:
	        ...
	        )
	{
		...
		{
			current = json_object_new_double(is_negative ? -json_Infinity() : json_Infinity()); 
	        ...
	        )
	{
		...
		{
			current = json_object_new_double( json_NaN() );
			saved_state = json_tokener_state_finish;
			state = json_tokener_state_eatws;
			goto redo_char;
		}
